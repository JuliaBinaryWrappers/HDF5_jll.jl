    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("HDF5_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "HDF5_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"HDF5", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "HDF5_jll", "src", "HDF5_jll.jl"))
        @info("HDF5_ll dev'ed out to /depot/dev/HDF5_jll with pre-populated override directory")
    end
end
